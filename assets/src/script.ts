const setImageSize = (container: HTMLDivElement, img: HTMLImageElement) =>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!containers) return console.log("no container available");
    ...
    for (const container of containers) {
        ...
        //draggable
        const divider = container.querySelector<HTMLDivElement>(".divider");
        if (!divider) continue;
    ...
    //if yes, animate it
    function checkPosition(containers: NodeListOf<HTMLDivElement>) {
        for (const container of containers) {
            const containerPosition = container.getBoundingClientRect().top;
            if (halfWindow + window.scrollY > containerPosition) container.classList.add("is-visible");
        }
        scrolling = false;
    }
